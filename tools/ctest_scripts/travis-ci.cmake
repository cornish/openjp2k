...
ctest_build(BUILD "${CTEST_BINARY_DIRECTORY}" RETURN_VALUE _build_rc)
# Dump build errors verbosely so CI surfaces actual diagnostics rather
# than the "N compiler errors" summary line.
if(NOT _build_rc EQUAL 0)
    file(GLOB _err_logs "${CTEST_BINARY_DIRECTORY}/Testing/Temporary/LastBuild_*.log")
    foreach(_log ${_err_logs})
        message("====== ${_log} ======")
        file(READ "${_log}" _log_contents)
        message("${_log_contents}")
    endforeach()
    message(FATAL_ERROR "Build failed with ${_build_rc} error(s); see above.")
endif()
if(NOT "$ENV{OPJ_CI_SKIP_TESTS}" STREQUAL "1")
